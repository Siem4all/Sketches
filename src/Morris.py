...
class CntrMaster(object):
    ...
    def num2cntr(self, targetVal):
        ...
        cntrLoInt = int(
            math.floor(math.log(1 + targetVal / self.a) * self.num2cntrNormFactor))  # lower-bound on the cntr
        ...
        if (settings.VERBOSE_DETAILS in verbose):
            if (len(optionalModifiedCntr) == 1):
                ...
                print('targetVal={}, cntrLoVec={}, cntrLoVal={:.2f}\n  cntrHiVec={}, cntrHiVal={:.2f}'.format
                      (targetVal, optionalModifiedCntr[0]['cntrVec'], optionalModifiedCntr[0]['val'],
                       optionalModifiedCntr[1]['cntrVec'], optionalModifiedCntr[1]['val']))
        if (
