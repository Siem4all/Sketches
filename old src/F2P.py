...
class CntrMaster(object):
    ...
    genSettingsStr = lambda self: '{}_n{}_h{}'.format(self.mode, self.cntrSize,
                                                      (self.hyperSize if (self.mode == 'F2P') else self.hyperMaxSize))
    ...
    printCntrLine = lambda self, cntr, expVec, expVal, mantVec, mantVal, cntrVal: print(
        'hyperVec={}, expVec={}, bias={}, expVec={}, mantVec={}, mantVal={} \nmantMinSize={}, offset={}, val={}'
        .format(cntr[0:self.hyperSize], expVec, self.bias, mantVec, mantVal, self.mantMinSize,
    ...
    expVec2expVal = lambda self, expVec, expSize: self.biasOfExpSize[expSize] - (
        int(expVec, base=2) if expSize > 0 else 0)

    # Given the value of the exponent, return the exponent vector representing this value 
    expVal2expVec = lambda self, expVal, expSize: np.binary_repr(num=int(self.biasOfExpSize[int(expSize)]) - expVal,
    ...
    def calcProbOfInc1F2P(self):
        ...
        for i in range(2 ** self.cntrSize):
            ...
            if (mantVal < (1 << self.mantSizeOfExpVal[expVal]) - 1):  # can still inc. the mantissa
                ...
                self.probOfInc1[i] = 1 / (
                            self.valOf(offset=self.offsetOfExpVal[expVal + 1], mantVal=0, expVal=expVal + 1) - cntrVal)
            if (settings.VERBOSE_RES in self.verbose):
    ...
    def calcOffsets(self):
        ...
        self.offsetOfExpVal = np.zeros(
            self.bias + 1)  # self.offsetOfExpVal[j] will hold the value to be added to the counter when the exponent is j
        for expVal in range(self.bias):  # for each potential exponent value
            self.offsetOfExpVal[expVal + 1] = self.offsetOfExpVal[expVal] + 2 ** (
                        expVal + self.mantSizeOfExpVal[expVal])
    ...
    def calcMantSizeOfExpVal(self):
        ...
        if (self.mode == 'F2P'):
            self.mantSizeOfExpVal = np.array(
                [self.mantMinSize + self.rangeOfExpVal[expVal] for expVal in range(self.bias + 1)])
        else:
            self.mantSizeOfExpVal = np.array(
                [self.mantMinSize + 2 * self.rangeOfExpVal[expVal] - 1 for expVal in range(self.bias + 1)])
    ...
    def calcParams(self):
        ...
        self.biasOfExpSize = np.ones(
            self.expMaxSize + 1)  # self.biasOfExpSize[j] will hold the bias to be added when the exp size is j
    ...
    def __init__(self, cntrSize, hyperSize, hyperMaxSize, mode, numCntrs, verbose):
        ...
        if (self.mode == 'F2P'):
            ...
            exit()
        if (
        not self.calcParams()):  # parameters couldn't be calculated, e.g. due to wrong given combination of cntrSize and hyperSize
            self.isFeasible = False
            return
        self.calcCntrMaxVal()
        self.rstAllCntrs()
    ...
    def cntr2num(self, cntr, hyperSize=None, hyperMaxSize=None, verbose=[]):
        ...
        if (
            ...
            print('the size of the given counter is {} while CntrMaster was initialized with cntrSize={}'.format(
                len(cntr), self.cntrSize))
            exit()
    ...
    def calcNprintCntr(self, cntr, expVec, expSize, mantVec):
        ...
        if (settings.VERBOSE_DEBUG in self.verbose):
            if (expVec != self.expVal2expVec(expVal, expSize=expSize)):
                print('error: expVec={}, expVal={}, expSize={}, Back to expVec={}'.format(expVec, expVal, expSize,
    ...
    def cntr2numF3P(self, cntr, hyperMaxSize=None):
        ...
        return self.calcNprintCntr(cntr=cntr, expVec=cntr[expVecBegin: expVecBegin + expSize], expSize=expSize,
    ...
    def cntr2numF2P(self, cntr, hyperSize):
        ...
        self.hyperVec = cntr[0:self.hyperSize]
        expSize = int(self.hyperVec, base=2)
        return self.calcNprintCntr(cntr=cntr, expVec=cntr[self.hyperSize:self.hyperSize + expSize], expSize=expSize,
    ...
    def incCntrBy1(self, cntrIdx):
        ...
        if (self.cntrs[cntrIdx] == self.cntrMaxVec or random.random() > self.probOfInc1[
            int(self.cntrs[cntrIdx], base=2)]):
    ...
    def incCntr(self, cntrIdx, mult=False, factor=1, verbose=[]):
        ...
        if (
            ...
            if (random.random() < probOfFurtherInc):
                ...
                self.cntrs[cntrIdx] = optionalModifiedCntr[0]['cntrVec']
        return math.log(int(self.cntr2num(self.cntrs[cntrIdx])))
    ...
    def num2cntr(self, targetVal, verbose=None):
        ...
        if (targetVal > self.cntrMaxVal):
            if (settings.VERBOSE_NOTE in self.verbose):
                print('Note: the requested cntr value {} is above the max feasible cntr for this configuration'.format(
                    targetVal))
    ...
    def calcCntrMaxValF3P(self):
        ...
        self.cntrZeroVec = np.binary_repr(2 ** self.cntrSize - 2 ** (self.cntrSize - 2 * self.hyperMaxSize),
                                          self.cntrSize)
    ...
    def calcCntrMaxValF2P(self):
        ...
        self.cntrZeroVec = np.binary_repr(2 ** self.cntrSize - 2 ** (self.cntrSize - self.hyperSize - self.expMaxSize),
...
def printAllCntrMaxVals(mode='F3P', hyperSizeRange=None, hyperMaxSizeRange=None, cntrSizeRange=[],
